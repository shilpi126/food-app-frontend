...
function Profile() {
     ...
     const { user } = useAuth();
    const [users,setUsers] = useState(JSON.parse(localStorage.getItem("user")).data);
    function data(){
        ...
        console.log(users);
        nameSet(users?.name)
        passwordSet(users?.password)
        emailSet(users?.email)
        passwordCnfSet(users?.password)
